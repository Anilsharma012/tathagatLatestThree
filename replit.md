...
- **Student Dashboard**: Professional widget design with 4 stat cards (Enrolled Courses, Completion Rate, Tests Taken, Lessons Done), Weekly Activity chart with gradient fill, Upcoming Classes with merged data from both dashboard API and live classes cache (fixed override bug), Course Progress doughnut with legend. Backend metrics (StudentDashboardController.js) integrate UserProgress model for lesson completion tracking alongside mock test data. Widget CSS classes: `widget-header`, `widget-empty-state`, `chart-wrapper`, `cp-*`.
- **Analysis & Reports**: Professional CSS-class-based design (Dashboard-analysis.css) with stat cards using gradient icons, ranking section, accuracy doughnut ring, performance trend chart, section-wise bar chart comparison, section comparison cards with accuracy bars, styled test attempts table with accuracy pills and action buttons, leaderboard modal with proper close button. Responsive grid layout.
